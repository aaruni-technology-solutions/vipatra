...
const InvoiceCreatePage = () => {
    ...
    // --- State Variables (Keep all your existing state) ---
    ...
    const [currentStep, setCurrentStep] = useState(1); // For stepper UI, not form visibility now
    const totalSteps = 5; // Total steps for the conceptual stepper

    // --- Handlers & Logic (Keep all your existing functions: handleLineItemChange, addLineItem, etc.) ---
    const handleLineItemChange = (index, field, value) => { /* ... */ };
    const addLineItem = () => { /* ... */ };
    const removeLineItem = (index) => { /* ... */ };
    const calculateTotals = useCallback(() => { /* ... */ }, [lineItems, overallDiscountValue, overallDiscountType]);
    ...
    const stepNames = [
        ...
        t('invoiceCreate.stepper.step4Name', 'Summary & Options'), // Combined Tax/Discount with Summary/Notes
        t('invoiceCreate.stepper.step5Name', 'Finalize') // Renamed for clarity
    ];

    const StepperUI = ({ current, total, names }) => (
        <div className="mb-10 hidden sm:flex justify-between items-start font-sans text-sm px-4 md:px-8 lg:px-0"> {/* Adjusted padding */}
            {names.slice(0, total).map((stepName, i) => (
                <div
                    ...
                    style={{ flexBasis: '19%' }} // Adjusted flex-basis
                >
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <main className="flex-1 p-6 sm:p-8 overflow-y-auto">
                    ...
                    {/* Stepper remains at the top */}
                    <StepperUI current={currentStep} total={totalSteps} names={stepNames} />

                    {/* Single Card for the entire form */}
                    <section className="dashboard-card">
                        <form id="invoiceForm" onSubmit={handleSubmit} className="space-y-8"> {/* Increased default space between logical groups */}

                            {/* Section 1: Invoice Type & Customer Details (Combined for better flow) */}
                            <div>
                                <h3 className="form-legend">{stepNames[0]} & {stepNames[1]}</h3> {/* Combined title */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                    ...
                                    <div> {/* Empty div for grid alignment or add another field here */} </div>

                                    {/* Customer Details */}
                                    <div className="md:col-span-2 pt-4"> {/* Add some top padding */}
                                        ...
                                        {/* Collapsible "Add New Customer" or always visible */}
                                        <details className="group">
                                            <summary className="text-sm font-medium text-primary hover:text-accent cursor-pointer list-none flex items-center">
                                            ...
                                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                                <div>
                                                    <label htmlFor="customerName" className="block text-sm font-medium text-primary mb-1 font-sans">{t('common.fullName')} <span className="text-danger-DEFAULT">{t('common.required')}</span></label>
                                                ...
                                                <div><label htmlFor="customerEmail" className="block text-sm font-medium text-primary mb-1 font-sans">{t('common.emailAddress')}</label><input type="email" id="customerEmail" name="customerEmail" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} className="form-element" /></div>
                                                <div><label htmlFor="customerPhone" className="block text-sm font-medium text-primary mb-1 font-sans">{t('common.phoneNumber')}</label><input type="tel" id="customerPhone" name="customerPhone" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className="form-element" /></div>
                                                <div className="md:col-span-2"><label htmlFor="customerAddress" className="block text-sm font-medium text-primary mb-1 font-sans">{t('common.addressOptional')}</label><textarea id="customerAddress" name="customerAddress" value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} rows="2" className="form-element"></textarea></div>
                                            </div>
                                        </details>
                                    </div>
                                </div>
                            ...
                            <div className="form-section-divider">
                                <h3 className="form-legend">{stepNames[2]}</h3>
                                <div id="invoiceItemsContainer" className="space-y-3"> {/* Reduced space-y */}
                                    {lineItems.map((item, index) => (
                                        <div key={item.id} className="invoice-item-row"> {/* Using new CSS class */}
                                            ...
                                            <div className="col-span-4 md:col-span-2 lg:col-span-1">
                                                ...
                                                <input type="number" name="itemQty" value={item.itemQty} onChange={(e) => handleLineItemChange(index, 'itemQty', e.target.value)} min="1" className="form-element-sm" />
                                            </div>
                                            <div className="col-span-4 md:col-span-2 lg:col-span-2">
                                                <label className="block text-xs font-medium text-primary mb-0.5 font-sans">{t('invoiceCreate.step3.price')}</label>
                                                <input type="number" name="itemPrice" value={item.itemPrice} onChange={(e) => handleLineItemChange(index, 'itemPrice', e.target.value)} placeholder="0.00" step="0.01" className="form-element-sm" />
                                            </div>
                                            <div className="col-span-4 md:col-span-2 lg:col-span-2">
                                                <label className="block text-xs font-medium text-primary mb-0.5 font-sans">{t('invoiceCreate.step3.tax')}</label>
                                                <input type="number" name="itemTax" value={item.itemTax} onChange={(e) => handleLineItemChange(index, 'itemTax', e.target.value)} placeholder="e.g. 18" className="form-element-sm" />
                                            </div>
                                             <div className="col-span-12 md:col-span-2 lg:col-span-1 text-right self-center pt-4 md:pt-0"> {/* Total for item - conceptual */}
                                                <p className="text-sm font-semibold text-primary">₹{((parseFloat(item.itemQty) || 0) * (parseFloat(item.itemPrice) || 0) * (1 + (parseFloat(item.itemTax) || 0)/100)).toFixed(2)}</p>
                                            </div>
                                            <div className="col-span-12 md:col-span-1 flex justify-end md:self-center pt-2 md:pt-0">
                            ...
                            {/* Section 3: Summary, Notes, e-Invoicing (Combined Step 4 from stepper) */}
                            <div className="form-section-divider">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6">
                                    ...
                                    <div className="lg:col-span-1 space-y-2 pt-1 font-sans text-sm bg-background p-4 rounded-lg border border-borderLight">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex">
                                                <input type="number" id="overallDiscountValue" name="overallDiscountValue" value={overallDiscountValue} onChange={(e) => setOverallDiscountValue(e.target.value)} className="form-element-sm !p-1.5 text-right rounded-r-none" style={{ width: '60px' }} />
                                                <select id="overallDiscountType" name="overallDiscountType" value={overallDiscountType} onChange={(e) => setOverallDiscountType(e.target.value)} className="form-element-sm !p-1.5 rounded-l-none border-l-0" style={{ width: '50px' }}>
                            ...
                            {/* Section 4: Preview & Generate (Combined Step 5 from stepper) */}
                            <div className="form-section-divider">
                                <h3 className="form-legend">{stepNames[4]}</h3> {/* Title: Finalize / Preview & Send */}
                                <div className="mb-6 p-4 border border-borderDefault rounded-lg bg-background min-h-[150px] flex items-center justify-center"> {/* Reduced min-h */}
                    ...
                    </section> {/* End of the single dashboard-card for the form */}
                </main>
            </div>
            <Footer />
        </div>
